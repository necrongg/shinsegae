...
function customSearch(container) {
    ...
    checkClose.type = 'checkbox';
    checkClose.id = 'toggleCloseEl';
    checkClose.title = '조회테스트';

    const label = document.createElement('label');
    label.htmlFor = 'toggleCloseEl';
    label.textContent = '조회테스트';
    label.style.marginLeft = '4px';
    ...
    function toggleCloseElDisplay(checked) {
        ...
        if (titleElToggle){
            unsafeWindow.search = function(popWin) {
                ...
                //2020.06.09 147627 이마트센터입고일 조회조건 추가 Start
                var rcvDate = getSearchConditionValue("EMART_CENTER_RCV_DATE");
                var outDate = getSearchConditionValue("OUTB_ECT_DATE");
                var param = "";

                //이마트센터입고일 O, 출고일자 O
                if((rcvDate == null || rcvDate == "") && (outDate == null || outDate == "")){
                    Ext.MessageBox.show({
                        msg: Lang.get('MSG_NOT_SELECTED_OUTB_RCV_DATE'),
                        buttons: Ext.MessageBox.OK,
                        icon: Ext.MessageBox.ERROR
                    });
                    return;
                    //이마트센터입고일 O, 출고일자 X
                }else if(rcvDate != null && (outDate == "" || outDate == null)){
                    param = {	WH_CFG_CD  : "OUTB_DISPATCH_CMPT_USE",
                        EMART_CENTER_RCV_DATE : getSearchConditionValue('EMART_CENTER_RCV_DATE').format("yyyyMMdd")
                    };
                }else{
                    param = {QPS_SEQ_EXISTS_YN : getSearchConditionValue("QPS_SEQ_EXISTS_YN")};
                }

                lalocOrderHeaderGrid.search(viewPort, '/lalocOrderDetailService/search', param, false, function(result, successYn){
                    if(popWin) popWin.close();
                    if(successYn && result["DS_OUT"].length > 0){
                        dispatch_cmpt_use = result["OUTB_DISPATCH_CMPT_USE"];
                        if(dispatch_cmpt_use == 'Y' ){
                            if(result["DS_OUT"][0]["DISPATCH_CMPT_YN"]!= dispatch_cmpt_use){
                                lalocOrderHeaderGrid.getSelectionModel().deselectAll();
                            }
                        }
                        //searchDetail();
                    }
    ...
    // 체크박스 이벤트
    checkClose.addEventListener('change', () => {
        const checked = checkClose.checked;
        toggleCloseElDisplay(checked);
    });
